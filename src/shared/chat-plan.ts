...
const current =
  /(?:latest|recent|today|news|weather|price|version|release|最新|最近|今天|新闻|天气|价格|版本|current\s+(?:version|release|price|status|news|information)|当前(?:版本|发布|价格|状态|消息|资讯)|现在(?:价格|状态|消息|资讯))/i;
const explicitWeb = /(?:search (?:the )?web|look (?:it )?up|online|internet|website|official site|web search|全网|上网|联网|网页|官网|搜索|检索|查一下|查找消息)/i;
...
export function planChatRequest(question: string, webSearchEnabled: boolean, language: "zh" | "en" = "en"): ChatPlan {
  ...
  const useWeb = webSearchEnabled || explicitWeb.test(text) || current.test(text);
